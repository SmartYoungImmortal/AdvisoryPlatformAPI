...
- Private SeaweedFS-backed avatar upload, replacement, removal, and owner-only five-minute download
...
- Unit, Postgres integration, and cookie-preserving auth e2e suites. E2e tests keep the application,
  authentication, and database real while replacing the external SeaweedFS boundary with an in-memory
...
The local SeaweedFS S3 gateway is at `http://localhost:8333`, and its filer UI is at
`http://localhost:8888`; its development-only credentials are in `.env.example`.
